...
import com.ibrahimcanerdogan.masterplayerguide.util.AnimationUtil
...
import com.ibrahimcanerdogan.masterplayerguide.view.adapter.weapon.WeaponSkinPagerAdapter
...
import com.ibrahimcanerdogan.masterplayerguide.databinding.FragmentWeaponDetailBinding
...
class WeaponDetailFragment : Fragment() {
    ...
    private fun setWeaponDetailData() {
        ...
        viewModel.weaponDetailData.observe(viewLifecycleOwner) {response ->
            when(response) {
                is Resource.Success -> {
        ...
        binding.apply {
            ...
            if (weaponData?.weaponDisplayName != "Melee") {
                ...
                includeWeaponDetailStat.apply {
                    ...
                    llWeaponDetailStats.setOnClickListener {
                        ...
                        expandableLayoutWeaponStats.isExpanded =
                            !expandableLayoutWeaponStats.isExpanded
                    }
                    weaponData?.weaponStats?.let {
                        textViewWeaponDetailStatFireRate.text =
                            getString(R.string.fire_rate, it.statFireRate.toString())
                        textViewWeaponDetailStatMagazineSize.text =
                            getString(R.string.magazine_size, it.statMagazineSize.toString())
                        textViewWeaponDetailStatRunSpeed.text =
                            getString(R.string.run_speed, it.statRunSpeedMultiplier.toString())
                        textViewWeaponDetailStatReloadSecond.text =
                            getString(R.string.reload_second, it.statReloadTimeSeconds.toString())
                        textViewWeaponDetailStatFirstBulletAcc.text = getString(
                            R.string.first_bullet_accuracy,
                            it.statFirstBulletAccuracy.toString()
                        )
                        textViewWeaponDetailStatEquipTime.text =
                            getString(R.string.equip_time, it.statEquipTimeSeconds.toString())
                        textViewWeaponDetailStatShotgunPelletCount.text = getString(
                            R.string.shotgun_pellet_count,
                            it.statShotgunPelletCount.toString()
                        )
                        textViewWeaponDetailStatWallPenetration.text = getString(
                            R.string.wall_penetration,
                            it.statWallPenetration!!.split("::")[1]
                        )
                    }
                }
                // DAMAGE
                includeWeaponDetailDamage.apply {
                    ...
                    llWeaponDetailDamage.setOnClickListener {
                        ...
                        expandableLayoutWeaponDamage.isExpanded =
                            !expandableLayoutWeaponDamage.isExpanded
                    }
                ...
                includeWeaponDetailShop.apply {
                    ...
                    llWeaponDetailShopData.setOnClickListener {
                        ...
                        expandableLayoutWeaponShopData.isExpanded =
                            !expandableLayoutWeaponShopData.isExpanded
                    }
                    weaponData?.let {
                        textViewWeaponDetailShopCost.text =
                            getString(R.string.shop_cost, it.weaponShopData?.shopCost.toString())
                        textViewWeaponDetailShopCategory.text =
                            getString(R.string.shop_category, it.weaponShopData?.shopCategory)
                        if (!it.weaponShopData?.shopImage.isNullOrEmpty()) {
                            Glide.with(binding.root.context)
                                .load(it.weaponShopData?.shopImage)
                                .into(imageViewWeaponDetailShopIcon)
                        } else if (!it.weaponShopData?.shopNewImage.isNullOrEmpty()) {
                            Glide.with(binding.root.context)
                                .load(it.weaponShopData?.shopNewImage)
                                .into(imageViewWeaponDetailShopIcon)
                        } else if (!it.weaponShopData?.shopNewImage2.isNullOrEmpty()) {
                            Glide.with(binding.root.context)
                                .load(it.weaponShopData?.shopNewImage2)
                                .into(imageViewWeaponDetailShopIcon)
                        } else {
            ...
            includeWeaponDetailSkin.apply {
                ...
                for (i in 0 until tabLayoutWeaponDetailSkin.tabCount) {
                    tabLayoutWeaponDetailSkin.getTabAt(i)!!.text =
                        weaponData.weaponSkins[i].skinDisplayName
                }
            }
        }
    }
